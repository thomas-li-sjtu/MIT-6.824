...
type RequestVoteArgs struct {
	...
	Term            int // 申请选举的任期
	Candidate_index int // candidate的任期
	Last_log_term   int // term of candidate's last log entry
	Last_log_index  int // term of candidate's last log entry
}
...
func (rf *Raft) change_state(change_to int, reset_timestamp bool) {
	...
	if change_to == TO_LEADER {
		...
		for i := range rf.next_index { // leader始终是最新的，next_index表明接下来要给follower发送的log index
			rf.next_index[i] = rf.get_last_index() + 1
		}
		rf.match_index = make([]int, len(rf.peers))
		rf.match_index[rf.me] = rf.get_last_index() + 1 // match_index表明，某一follower已经存储到的log index
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.cur_term > args.Term {
		...
		if (rf.voted_for == -1 || rf.voted_for == args.Candidate_index) &&
			...
			rf.cur_term == args.Term {
			// fmt.Println(rf.me, "vote for ", args.Candidate_index)
		...
		go func(server_id int) {
			...
			if ok {
				if rf.state != "candidate" || vote_args.Term != rf.cur_term {
					...
					// fmt.Println("candidate", rf.me, "state has changed")
					return
				}
				if vote_reply.Term > rf.cur_term {
					// rule1不满足
					// fmt.Println(rf.me, "cur_term less than", server_id)
